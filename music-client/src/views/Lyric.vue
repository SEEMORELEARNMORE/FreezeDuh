<template>
  <div class="song-container">
    ...
    <ul class="song-info">
      <li>歌手：{{ singerName }}</li>
      <li>歌曲：{{ songTitle }}</li>
      <li>播放量：{{ clickcount }}</li>
      <li>收藏量：{{ collectioncount }}</li>
    </ul>
  </div>
  <div class="container">
...
export default defineComponent({
  ...
  setup() {
    ...
    const songId = computed(() => store.song.songId) // 歌曲ID
    const lyric = computed(() => store.song.lyric) // 歌词
    const currentPlayList = computed(() => store.song.currentPlayList) // 存放的音乐
    const currentPlayIndex = computed(() => store.song.currentPlayIndex) // 当前歌曲在歌曲列表的位置
    const curTime = computed(() => store.song.curTime)
    const songTitle = computed(() => store.song.songTitle) // 歌名
    const collectioncount = computed(() => store.song.collectioncount) //收藏量
    const clickcount = computed(() => store.song.clickcount) // 播放量
    const singerName = computed(() => store.song.singerName) // 歌手名
    const songPic = computed(() => store.song.songPic) // 歌曲图片
    watch(songId, () => {
        ...
        for (let i = 0; i < lyricArr.value.length; i++) {
          if (curTime.value >= lyricArr.value[i][0]) {
            for (let j = 0; j < lyricArr.value.length; j++) {
              if (document.querySelectorAll('.has-lyric li')[j]) {
            ...
            if (i >= 0) {
              ...
              if(document.querySelectorAll('.has-lyric li')[i]){
