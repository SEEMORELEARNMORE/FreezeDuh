...
public class CollectController {
    ...
    //添加多个收藏歌曲
    @PostMapping("/collection/addmany")
    public R addCollectionMany(@RequestBody CollectRequest[] addCollectRequest){
        int i;
        for(i=0;i<addCollectRequest.length;i++)
        {
             collectService.addCollection(addCollectRequest[i]);
        }
        return R.success("添加完毕");
    }
...
//    //返回指定歌曲的收藏数
//    @GetMapping("collection/songid/count")
//    public R collectionOfSong(@RequestParam Integer songId){return collectService.collectionOfSong(songId);}
}
