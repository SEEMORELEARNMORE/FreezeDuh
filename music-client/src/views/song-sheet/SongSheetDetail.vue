...
export default defineComponent({
  ...
  setup() {
    ...
    const songDetails = computed(() => store.song.songDetails) // 单个歌单信息
    const nowUserId = computed(() => store.user.userId)
    ...
    async function getUserRank(userId, songListId) {
      if (store.configure.token) {
        console.log(`用户id：${userId},歌单id：${songListId}`)
