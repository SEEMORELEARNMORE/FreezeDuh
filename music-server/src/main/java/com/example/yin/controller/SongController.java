...
public class SongController {
    ...
    public R songOfId(@RequestParam int id) {
        //更新收藏
        songService.updateSongCollectionCount(id);
        //更新点击量
       // songService.updateSongClickCount(id);
        //更新评论
        songService.updateSongCommentCount(id);
    ...
    public R songOfSongName(@RequestParam String name) {return songService.songOfSongName('%'+name+'%');}

    //返回指定专辑的所有歌曲
    @GetMapping("/song/introduction/detail")
    public R songOfIntroduction(@RequestParam String introduction){return songService.songOfIntroduction('%'+introduction+'%');}
    ...
    //修改歌曲点击量
    @PostMapping("/song/clickcount/update")
    public R updateSongClickCount(@RequestParam("id") int id){
        return songService.updateSongClickCount(id);
    }

    //根据点击量排行
    @GetMapping("/rank/song/clickcount")
    public R rankSongByClickCount()
    {
        return songService.rankSongByClickCount();
    }
    //根据收藏量排行
    @GetMapping("/rank/song/collectioncount")
    public R rankSongByCollectionCount()
    {
        return songService.rankSongByCollectionCount();
    }
    //根据评论量排行
    @GetMapping("/rank/song/commentcount")
    public R rankSongByCommentCount()
    {
        return songService.rankSongByCommentCount();
    }
}
