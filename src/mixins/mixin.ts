...
export default function () {
  ...
  function checkStatus(status?: boolean) {
    if (!Cookies.get('token') && arguments.length == 0) {
  ...
  // 播放
  function playMusic({ id, url, pic, index, name, lyric,collectioncount,clickcount, currentSongList }) {
    const songTitle = getSongTitle(name);
    const singerName = getSingerName(name);
    store.song.playMusic({
      id,
      url,
      pic,
      index,
  ...
  function changeIndex(value) {
    store.configure.setActiveNavName(value);
  }
  // 路由管理
  ...
  function debounce(this:any,func: Function) {
