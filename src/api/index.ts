...
const HttpManager = {
  ...
  // =======================> 用户 API 完成
  // 登录
  signIn: ({username,password}) => post(`UserLogin`, {username,password}),
  // 注册
  ...
  // 歌单中的歌曲全部收藏
  collectionAll:(data)=>post(`collection/addmany`,data),
  ...
  // 返回指定歌手名的歌手
  getSinger: (keywords) => get(`singer/name/detail?name=${keywords}`),
  // 返回指定歌手名和歌曲名的歌曲
  getSongOfSingerSongName: (keywords) => get(`song/singerName/detail?name=${keywords}`),
  // getSongOfS
  // 下载音乐
  ...
  // 根据id获取用户信息 
  getUserInfo:()=>post("/getMessage"),

  // 点击歌曲 点击量增大
  addClicks:(id)=>post(`song/clickcount/update?id=${id}`),

  // 排行榜

  // 播放量
  clickBoard:()=>get('rank/song/clickcount'),

  // 收藏榜
  collectionBoard:()=>get('rank/song/collectioncount'),

  // 评论榜
  commentBoard:()=>get('rank/song/commentcount'),

  // 歌单评分榜
  songlistBoard:()=>get('rank/songlist/score')
};
